...
pr_x = np.array(feature_predict).reshape(-1, 1)
...
predictions = encoder.inverse_transform(model.predict(pr_x))
# print(encoder.inverse_transform(model.predict(np.array([[3]]))))
predictions_proba = model.predict_proba(pr_x)
